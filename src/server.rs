...
pub async fn run(socket_addr: SocketAddr) {
    ...
    let base_uri = Arc::new(unwrap_or_abort!(Uri::try_from(format!(
        "http://{}",
        local_addr
    ))));
...
async fn process_request(
    ...
    Ok(match method {
        ...
        Method::POST => {
            ...
            let content_type: Option<&str> = match headers
                .get("Content-Type")
                .map(HeaderValue::to_str)
            {
                ...
                Some(Err(_)) => return Ok(bad_request("Invalid ASCII in Content-Type header.")),
            };

            match PostParams::parse(query) {
                // Client wants to store a static file of a known Content-Type:
                Some(PostParams::StaticPage) => {
                    page.set_static(content_type.map(String::from), body_bytes)
                        .await;
                    Response::new(Body::empty())
                }
                // Client wants to publish some HTML to a dynamic page:
                Some(PostParams::DynamicPage { title }) => match String::from_utf8(body_bytes) {
                    Ok(body) => {
                        if cfg!(debug_assertions) {
                            eprintln!("\n{}", body);
                        }
                ...
                Some(PostParams::SubscribeEvents { uuid }) => {
                    if let Ok(subscription) = serde_json::from_slice(&body_bytes) {
                        ...
                        if let Some((uuid, body)) = page.event_stream(uuid, subscription).await {
                            ...
                            return Ok(bad_request(
                                "Invalid resubscription request: stream does not exist.",
                            ));
                        }
                    } else {
